...
RCT_EXPORT_METHOD(setupNotification:(NSInteger)types) {
    ...
    RCTLogInfo(@"setupNotification success");
}
...
RCT_EXPORT_METHOD(setBadgeCounts:(NSInteger)badgeCount) {
...
RCT_EXPORT_METHOD(openNotifications) {
  [MobPush openSettingsForNotification: ^(BOOL success) {
      PushRNDebugLog(@"MobPush openNotifications %@", @(success));
    }];
}

RCT_EXPORT_METHOD(isNotificationsEnabled:(RCTResponseSenderBlock)callback) {
    [MobPushModule main_async_callback:^{
        [MobPush requestNotificationAuthorizationStatus:^(MPushAuthorizationStatus status) {
          BOOL stop = status < MPushAuthorizationStatusDenied ? NO : YES;
            PushRNDebugLog(@"Current SDK Whether Stoped Push Service: %@", @(status));
            if (callback) callback(@[@{@"success": @(YES), @"res": @(stop)}]);
        }];
...
- (void)mobpushEventReminderReceived:(NSNotification *)notification {
    ...
    // 处理跳转事件（类似 Android 端的逻辑）
    if ([message messageType] == MPushMessageTypeClicked) {
        NSDictionary *userInfo = message.notification.userInfo;
        NSString *schemeUrl = userInfo[@"scheme_url"];
        
        // 匹配 boklock://message 时，发送跳转事件到RN层
        if (schemeUrl && [schemeUrl isEqualToString:@"boklock://message"]) {
            NSDictionary *jumpMap = @{
                @"pagePath": @"/pages/user/message/index"
            };
            [self sendEventWithName:@"jumpToPage" body:jumpMap];
        }
    }
}
