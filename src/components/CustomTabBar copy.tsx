...
// 预计算样式对象，避免重复创建
const PRESSED_STYLE = { opacity: 0.7 }
const NORMAL_STYLE = { opacity: 1 }
...
const TabItem = memo(function TabItem({
  ...
  const imageSource = useMemo(() => 
    isFocused ? tabConfig.imgUrlActive : tabConfig.imgUrl,
    [isFocused, tabConfig.imgUrlActive, tabConfig.imgUrl]
  )
...
export const CustomTabBar = memo(function CustomTabBar({ 
  ...
  const handleTabPress = useCallback((route: any, isFocused: boolean) => {
    ...
    // 防抖：300ms内重复点击同一路由，忽略
    if (now - lastPressTime.current < 300 && lastPressRoute.current === route.name) {
      return
    }
