...
const PageContainer = forwardRef<PageContainerRef, PageContainerProps>(
  (
    {
      children,
      // 样式默认值
    ...
    const renderNavHeader = useMemo(() => {
        ...
        return (
          <View
            style={[
              ...
              navBorder && styles.navHeaderBorder,
            ]}
          >
      ...
      // 否则渲染传入的 header 组件
      return header;
    }, [pageNavProps, header, backgroundImage, navigation, navBorder]);
    ...
    const renderContent = useMemo(() => {
      ...
      if (error && fullScreenError) {
        ...
        if (codeStr === String(LOGIN)) {
          return <StatusLogin key={reloadSeed} />;
        }
        return <StatusError key={reloadSeed} error={error} onRetry={onRetry} />;
      }
      ...
      // C. 普通视图
      return (
        <View key={reloadSeed} style={contentStyle}>
          {children}
        </View>
      );
    }, [
