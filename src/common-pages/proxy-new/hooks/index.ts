...
export function usePaging<T>(
  ...
  const refreshList = useCallback(() => {
    ...
    const promise = requestFn(pageNo.current, pageSize.current).then(result => {
      ...
      let total = result?.length ? pageNo?.current + 1 : pageNo?.current || 0;
      setTotalPage(total);
      setTotalSize(pageSize.current);
      setVersion(version + 1);
      setResultPageNo(pageNo.current);
      if (pageNo.current > 1) {
        setResultList([...resultList, ...(result as any)]);
      } else {
        setResultList(result as any);
      }
    });
