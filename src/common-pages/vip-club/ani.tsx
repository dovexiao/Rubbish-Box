...
const VipClubList: React.FC<VipClubListProps> = ({
  ...
  // VIP Card List refs and state
  const cardListRef = useRef<FlatList>(null);

  // VIP Club List refs and state
  ...
  const handleCardScroll = (e: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    if (onCheck && checkIndex !== clampedIndex) {
      onCheck(clampedIndex);
    }
  };

  // VIP Card List effect
  useEffect(() => {
    if (vipList.length <= 0) {
      return;
    }
  ...
  useEffect(() => {
    const id = scrollX.addListener(({value}) => {
      ...
      if (onCheck && checkIndex !== clamped) {
        onCheck(clamped);
      }
    });
    return () => scrollX.removeListener(id);
  }, [scrollX, vipConfigList.length, onCheck, checkIndex]);

  useEffect(() => {
    if ((checkIndex || checkIndex === 0) && scrollViewRef.current) {
      scrollViewRef.current.scrollTo({
        x: checkIndex * CARD_WIDTH,
        animated: true,
      });
    }
  }, [checkIndex]);
          ...
          {vipConfigList.map((item, index) => (
            <View
              ...
              style={{
                ...
                zIndex: index === activeIndex ? 10 : 1, // iOS
                elevation: index === activeIndex ? 10 : 1, // Android
              }}>
              {/* 三角形指示器 - 只显示在当前活跃卡片上 */}
              {index === activeIndex && (
                <View
