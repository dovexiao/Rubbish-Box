...
const WebViewInner: React.ForwardRefRenderFunction<
  WebViewInnerRef,
  WebViewOptions
> = (options, outRef) => {
  ...
  const processData = (data: string | BasicObject) => {
    ...
    if (typeof data === 'string') {
      ...
      if (data.startsWith('gotoUrl:')) {
        const paramsJsonStr = data.substring('gotoUrl:'.length);
        if (paramsJsonStr.startsWith('http')) {
          goToUrl(paramsJsonStr);
        } else {
          /* 路由name */
          goTo(paramsJsonStr);
        }
      }
      if (data.startsWith('PromotionDetail:')) {
        const paramsJsonStr = data.substring('PromotionDetail:'.length);
        const paramsArr: string[] = paramsJsonStr.split('?');
        let value = '';
        if (paramsArr[1]) {
          let strArr = paramsArr[1].split('=');
          value = strArr[1];
        }
        goTo(paramsArr[0], {id: value || 10});
      }
    }
    onMessage?.(data);
  };
...
export default forwardRef(WebViewInner);
