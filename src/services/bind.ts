 ...
 * 获取绑定二维码
 */
export async function getBindQRCode(): Promise<BindDeviceResponse> {
  // 获取设备信息
  const deviceInfo = await getDeviceInfoForAPI()
  console.log("📌 getBindQRCode: deviceInfo", deviceInfo)
  
  // 获取用户手机号（从存储中）
  const { getString, STORAGE_KEYS } = await import("../utils/storage")
  const userInfoStr = getString(STORAGE_KEYS.USER_INFO)
  let phone = ""
  ...
  if (userInfoStr) {
    try {
      ...
      phone = parsed.phone || ""
      console.log("📌 getBindQRCode: parsed user phone", phone)
    } catch (e) {
  ...
  try {
    ...
    const res = await axios.post<{ success: boolean; data: BindDeviceResponse; message?: string }>(
      url,
      requestData,
      {
        timeout: API_TIMEOUT,
